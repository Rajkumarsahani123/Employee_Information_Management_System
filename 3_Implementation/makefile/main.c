...
int main()
{
    ...
    */
    ...
    while(1)
    {
        ...
        case '2':
            system("cls");
            printf("EMPLOYEE's RECORD LIST (name, age, address, designation, department, salary)");
            rewind(fp); ///this moves file cursor to start of the file
            while(fread(&e,recsize,1,fp)==1)  /// read the file and fetch the record one record per fetch
            {
            	
                printf("\n\n%s \t\t%d \t%s \t%s \t%s \t%.2f",e.name,e.age,e.address,e.designation,e.department,e.bs); /// print the name, age and basic salary
            }
            getch();
            break;
        ...
        case '3':  /// if user press 3 then do editing existing record
            ...
            while(another == 'y')
            {
                ...
                while(fread(&e,recsize,1,fp)==1)  /// fetch all record from file
                {
                    ...
                    {
                        printf("\nEnter new name,age,address,designation,department and bs: ");
                        scanf("%s%d%s%s%s%f",&e.name,&e.age,&e.address,&e.designation,&e.department,&e.bs);
